...
-- 7. 初始化默认配置
INSERT INTO `$1`.`configs` (tenant_id, config_key, config_value, config_type, description, is_system) VALUES
($2, 'system.name', '租户管理系统', 'STRING', '系统名称', TRUE),
($2, 'timezone', 'Asia/Shanghai', 'STRING', '时区设置', TRUE),
($2, 'language', 'zh-CN', 'STRING', '语言设置', TRUE),
($2, 'max.users', '10000', 'NUMBER', '最大用户数限制', TRUE),
($2, 'max.storage.size', '107374182400', 'NUMBER', '最大存储空间(字节)', TRUE),
($2, 'session.timeout', '7200', 'NUMBER', '会话超时时间(秒)', TRUE);
